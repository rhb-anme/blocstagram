...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *lineScreenFilter = [CIFilter filterWithName:@"CILineScreen"];
        
        if (lineScreenFilter) {
            [lineScreenFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:lineScreenFilter.outputImage withFilterTitle:NSLocalizedString(@"Line Screen", @"Line Screen Filter")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *firstStage = [CIFilter filterWithName:@"CIGloom"];
        CIFilter *secondStage = [CIFilter filterWithName:@"CISpotLight"];
        ...
        if (firstStage)
        {
            ...
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Super Hipster", @"Super Hipster Filter")];
